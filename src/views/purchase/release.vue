		...
		<div class="limit-max-width-inset release-form">
			<el-form ref="Form" :model="form" label-width="155px" label-position="right" :rules="rules">
				...
				<el-form-item label="价格" prop="price">
					<el-input-number v-model="form.price" :controls="false" :precision="2" placeholder="请输入商品数量" @blur="setcount()" :min="0"></el-input-number>
				</el-form-item>
				...
				<el-form-item prop="goodsPic" label="商品图片"><uploadImg moduleName="ncq" ref="uploadImage" :limit="5" v-model="form.goodsPic"></uploadImg></el-form-item>
				...
				<el-form-item>
					...
					<div style="margin-top: 20px;">
						...
						<a style="color: #FF7E26;" @click="openMit">《平台供货协议》</a>
						<span>、</span>
						<a style="color: #FF7E26;">《供应链金融服务协议》</a>
						<span>。</span>
					</div>
				</el-form-item>
			</el-form>
...
export default {
				...
				count: [
					...
					},
					{
						validator: (rule, value, callback) => {
							let non = value <= 0;
							if (non) {
								callback(new Error('请设置大于正整数的数量'));
							} else {
								callback();
							}
						},
				...
				price: [
					...
					},
					{
						validator: (rule, value, callback) => {
							let non = value <= 0;
							if (non) {
								callback(new Error('请设置大于正整数的价格'));
							} else {
								callback();
							}
						},
	...
	methods: {
		...
		async cgrelease() {
			...
			this.form.goodsPic = JSON.stringify(imageList);
			let res = await ApiAuth.cgrelease({
				...
				goodsImageList: this.form.goodsPic,
